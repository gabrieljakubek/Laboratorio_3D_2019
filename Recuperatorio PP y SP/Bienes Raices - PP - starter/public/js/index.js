...
function inicializarManejadores() {
    ...
    limpiarForm();
    $("#limpiar").click(limpiarForm);
    frm.addEventListener('submit', guardar);
    $("#eliminar").click(bajaAnuncio);
    $("#selTransaccion").click(filtrarTabla);
    $("#eliminar").hide();
    cargarDatos(true);
    for (let key in ETransaccion) {
        if (!parseInt(key) && key != 0) {            
            enumeradores.push(key);
        }
    }
    cargarSelect($('#selTransaccion')[0],enumeradores);
...
function crearAnuncio(frm) {
    ...
    return new BienRaiz(id, titulo, transaccion, descripcion, precio, num_wc, num_estacionamiento, num_dormitorio);
}
...
function filtrarTabla() {
    ...
    $("#promedio").val(promedio(listado));
}

promedio = function(listados) {
    return listados
         .map(l => l._precio)
         .reduce((suma, _precio, indice, lista) => suma += _precio / lista.length, 0).toFixed(2);
